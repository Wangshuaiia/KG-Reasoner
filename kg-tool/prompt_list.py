relation_selection_prompt = """You are tasked with selecting the most relevant relations (edges) from a knowledge graph that contribute to answering the **Subquestion** while aligning with the **Original Question**. Focus on the **Subquestion** as the immediate goal for guiding the next step of reasoning, but ensure the selected relations also maintain relevance to the broader context of the **Original Question**.

**Guidelines:**  
1. **Analyze the Subquestion**: Carefully examine the **Subquestion** to determine the specific information being sought in this step.  
2. **Contextualize with the Original Question**: While prioritizing the **Subquestion**, ensure that the selected relations ultimately contribute to addressing the **Original Question's** overarching target.  
3. **Select 3 Relevant Relations**: From the provided list of relations, choose 3 relations that are most likely to yield relevant answers to the **Subquestion**.  
4. **Score the Contribution**: Assign a contribution score to each selected relation on a scale of 0 to 1. The score should reflect the degree to which a relation contributes to resolving the **Subquestion** in this reasoning step.  
5. **Provide Justification**: For each selected relation, briefly explain why it is relevant and how it contributes to addressing the **Subquestion.**

**Input Format:**  
- **Original Question:** {Original question text}  
- **Topic Entity:** {Initial topic or node of interest}  
- **Subquestion:** {Focused question for the current step of reasoning}
- **Relations:** {List of potential relations to evaluate}  

**Output Format:**  
1. List the selected relations with their respective scores and explanations in the following format:  
   - {relation (Score: X)}: Explanation of relevance and contribution.  
2. Ensure the total score equals 1.  

**Example:**  

**Input:**  
- **Original Question:** Name the president of the country whose main spoken language was Brahui in 1980?  
- **Topic Entity:** Brahui Language  
- **Subquestion:** Which country’s main spoken language is Brahui?  
- **Relations:** language.human_language.main_country; language.human_language.language_family; language.human_language.iso_639_3_code; base.rosetta.languoid.parent; language.human_language.writing_system; base.rosetta.languoid.languoid_class; language.human_language.countries_spoken_in; kg.object_profile.prominent_type; base.rosetta.languoid.document

**Output:**  
1. {language.human_language.main_country (Score: 0.5)}: This relation directly identifies the main country associated with the Brahui language, making it highly relevant to the Subquestion.  
2. {language.human_language.countries_spoken_in (Score: 0.3)}: This relation provides additional context about other countries where Brahui is spoken, which could help validate or expand the answer to the Subquestion.  
3. {base.rosetta.languoid.parent (Score: 0.2)}: This relation offers insights into the linguistic hierarchy, which might indirectly assist in narrowing down the country of interest.  

**Important Notes:**  
1. Prioritize relations that directly address the **Subquestion**.  
2. Avoid selecting relations that do not meaningfully contribute to narrowing the reasoning path.  
3. Justify each score clearly to demonstrate how the selected relations aid in progressing the reasoning task.  
4. **If the original question is already simple enough and does not require further decomposition, return the original question instead of generating a sub-question.**  

**Now, select the most relevant relations and score their contributions.**  
"""
